    ...
    while not QUIT:
        ...
        while GAME:
            ...
            if turnP1:
                ...
                deadMansThread.start()  ## INICIA CONTAR O TEMPO PARA RESPONDER
                col = NULL
                while col == NULL:
                    if fila.get() > 60:
                        result = 2
                        col = "TRIGGERACTIVATED"
                    col = g.playerInput(1, currentMatch)
                deadMansThread.join() ## FINALIZA A CONTAGEM DO TEMPO PARA RESPONDER
                time.sleep(0.5)
                if col == "TRIGGERACTIVATED":
                    GAME = False
                else:
...
if __name__ == '__main__':
    fila = queue.Queue()
    serverThread = threading.Thread(target=serverProgram(), args=(fila,))
